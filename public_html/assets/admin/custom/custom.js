    ...
    for (var i = 0; i < productVariantIds.length; i++) {
        ...
        if (delivered_quantity != quantity && data.active_status != "cancelled" && data.active_status != "delivered") {
            ...
            let row = "<tr>" +
                ...
                "<td>" + quantity + "</td>" +
                "<td>" + unit_price + "</td>" +
                `<td><label for="checkbox-${productVariantIds[i]}"><input type="checkbox" data-item-id="${orderItemId[i]}" name="checkbox-${productVariantIds[i]}" id="checkbox-${productVariantIds[i]}" class="form-control product-to-ship"></label></td>`
            "</tr>";
...
$('#transaction_modal').on('shown.bs.modal', function (e) {
    ...
    $('[name="consignment_id"]').val(consignment_id);
