...
$(document).ready(function () {
    $('#modal-signup').on('hidden.bs.modal', function () {
        ...
        // Show the #send-otp-form by default and remove any inline styles
        $('#send-otp-form').removeClass('d-none').removeAttr('style'); 

        // Ensure #verify-otp-form is hidden
...
function formatRepo(e) {
    ...
    if (e.suggestion_keyword) {
        s = "<div class='select2-result-repository clearfix'>" +
            ...
            "</a>" +
            "<button class='select2-result-repository__icon search_btn' onclick=\"copySearch('" + e.suggestion_keyword + "')\"><i class='font-weight-bold fs-20 uil uil-arrow-up-left'></i></button>" +
            "</div></div>";
    }
    ...
    $(document).on("click", "#user_logout", function (e) {
        ...
        Swal.fire({
            ...
            if (result.dismiss === Swal.DismissReason.cancel) {
                Swal.fire('Cancelled!', 'You have not logged out', 'error');
            }
        });
    }),
            ...
            success: function (e) {
                ...
                if (e.error == true) {
                    ...
                    if (r.hasClass("fa-heart-o")) {
                        ...
                        // r.addClass("fa-heart text-danger").css("color", "red");
                        ...
                        if (data.check_deliverability.city_wise_deliverability == 1) {
                            if (data.type != "digital_product") {
                                variant_attributes +=
                                    ...
                                    '</div>' +
                                    '<button type="submit" class="btn btn-sm ml-0 check-availability" style="background-color:#8b285d;color:white;" data-product_id="' + data.id + '"  data-city=""  id="validate_city">Check Availability</button>' +
                                    '</div>' +
                        ...
                        if (data.check_deliverability.pincode_wise_deliverability == 1) {
                            ...
                            if (data.type != "digital_product") {
                                variant_attributes +=
                                    ...
                                    '</div>' +
                                    '<button type="submit" class="btn btn-sm ml-0 check-availability" style="background-color:#8b285d;color:white;" data-product_id="' + data.id + '"  data-zipcode="' + data.zipcode + '"  id="validate_zipcode">Check Availability</button>' +
                                    '</div>' +
                                    '<div class="mt-2" id="error_box1">' +
                                    err_msg +
                                    ' </div>' +
        ...
        if (listnerElement != null) {
            document.getElementById("modal-signup").addEventListener("show.bs.modal", () => {
                ...
                e.intlTelInput({
                    ...
                    formatOnDisplay: !0,
                    autoFormat: !0,
                    // onlyCountries: ['in'], for set only selected countries in dropdown list
                    autoHideDialCode: !0,
                    autoPlaceholder: !0,
                    defaultCountry: "in",
                    ipinfoToken: "yolo",
