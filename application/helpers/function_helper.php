...
function fetch_product($user_id = NULL, $filter = NULL, $id = NULL, $category_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $return_count = NULL, $is_deliverable = NULL, $seller_id = NULL, $is_detailed_data = 0)
{
    ...
    if (!empty($product)) {
        ...
        for ($i = 0; $i < count($product); $i++) {
            ...
            // print_r(explode(',',$product[$i]['tax_percentage']));
            ...
            // print_r(gettype($product[$i]['total_tax_percentage']));
            // print_r("***");
            ...
            if (!empty($product[$i]['variants'])) {
                ...
                for ($k = 0; $k < count($product[$i]['variants']); $k++) {
                    ...
                    // print_r($product[$i]['tax_percentage']);
                    // print_r('*****');
            ...
            // Raghu - Copilot suggestions

            if (!empty($other_images)) {
                $k = 0;
                if (is_array($other_images_md)) {
                    ...
                    // Handle the case where $other_images_md is not an array
                    $product[$i]['other_images_md'] = [];
                }

                $k = 0;
                if (is_array($other_images_sm)) {
                    ...
                    // Handle the case where $other_images_sm is not an array
                    $product[$i]['other_images_sm'] = [];
                }
                ...
                if (is_array($other_images)) {
                    ...
                    // Handle the case where $other_images is not an array
...
function create_consignment($data)
{
    ...
    $product_variant_ids = [];
    $items = fetch_details('order_items', ['order_id' => $order_id], 'active_status,id,product_variant_id');
    ...
    //     $index = array_search($item['product_variant_id'], $product_variant_id);
    //     // $consignment_qty = (int)$ship_qty_input[$index];
    //     // $remaining_qty = (int)$item["quantity"] - (int)$item["delivered_quantity"];
    //     // if ($consignment_qty > $remaining_qty) {
    //     //     $error_message = "Maximum consignment quantity for Varient " . $item["product_variant_id"] . " is $remaining_qty";
    //     // }
...
function view_all_consignments($order_id = null, $consignment_id = null, $seller_id = null, $offset = null, $limit = null, $order = "DESC", $in_detail = 1, $delivery_boy_id = null, $multiple_status = null)
{
    $t = &get_instance();

    $count_res = $t->db->select('COUNT(DISTINCT(c.id)) as total')
        ->join('orders o', 'c.order_id = o.id', 'left')
        ->join('order_items oi', 'oi.order_id = c.order_id', 'left')
    ...
    $search_res = $t->db->select('u.username,u.email,u.mobile,u.latitude,u.longitude,u.image as user_profile,c.id, c.order_id,c.delivery_boy_id, c.name, c.status,c.active_status, o.date_added as created_date, c.otp, oi.seller_id,o.payment_method,o.address as user_address,o.delivery_charge,o.wallet_balance,o.discount,o.promo_discount,o.total_payable,o.notes,o.delivery_date,o.delivery_time,o.is_cod_collected,o.is_shiprocket_order,o.final_total,o.total')
        ->from('consignments c')
        ->join('orders o', 'c.order_id = o.id')
        ->join('order_items oi', 'oi.order_id = c.order_id')
    ...
    $consignment_list = [];
    $total_item_in_consignment = count($result);
    foreach ($result as $row) {
        ...
        // if (!isset($consignment_list[$c_id])) {
        $data = [
            'id' => $row['id'] ?? "",
            ...
            'user_profile' => base_url($row['user_profile']) ?? "",
            'total' => $row['total'] ?? "",
            'delivery_charge' => 0,
        ...
        foreach ($consignment_items as $item) {
            ...
            $data['tax_percent'] = "$total_tax_percent";
        ...
        $data['delivery_charge'] = "$order_item_delivery_charges";
...
function get_cart_total($user_id, $product_variant_id = false, $is_saved_for_later = '0', $address_id = '', $delivery_method = '')
{
    $t = &get_instance();
    ...
    $t->db->join('seller_commission sc ', 'sc.seller_id=p.seller_id');
...
function fetch_orders($order_id = NULL, $user_id = NULL, $status = NULL, $delivery_boy_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $download_invoice = false, $start_date = null, $end_date = null, $search = null, $city_id = null, $area_id = null, $seller_id = null, $order_type = '', $from_seller = false, $draftFilter = 1)
{
    ...
    $search_res = $t->db->select(' o.*, u.username,u.image as user_profile,u.country_code, u.email as email, p.name,p.type,p.download_allowed,p.pickup_location,a.name as order_recipient_person,pv.special_price,pv.price,oc.delivery_charge as seller_delivery_charge,oc.promo_discount as seller_promo_dicount')
        ->join(' `users` u', 'u.id= o.user_id', 'left')
        ->join(' `order_items` oi', 'o.id= oi.order_id', 'left')
        ->join('product_variants pv', 'pv.id=oi.product_variant_id', 'left')
        ->join('addresses a', 'a.id=o.address_id', 'left')
...
// Raghu
    ...
    for ($i = 0; $i < count($order_details); $i++) {
        ...
        (Select count(active_status) from order_items where active_status ="cancelled" and order_id = oi.order_id ) as order_cancel_counter , (Select count(active_status) from order_items where active_status ="returned" and order_id = oi.order_id ) as order_return_counter , c.otp as otp,ci.id as consignment_item_id')
            ->join('product_variants pv', 'pv.id=oi.product_variant_id', 'left')
            ->join('products p', 'pv.product_id=p.id', 'left')
...
function cancel_shiprocket_order($shiprocket_order_id)
{
    ...
    if ($res['status'] == 200 || $res['status_code'] == 200) {
        ...
        if ($old_active_status != "processed") {
...
function update_shiprocket_order_status($tracking_id)
{
    ...
    $order_tracking_details = fetch_details("order_tracking", ['tracking_id' => $tracking_id, 'is_canceled' => 0], 'order_id,consignment_id');
    ...
    if (isset($res[0]['tracking_data']) && !empty($res[0]['tracking_data'])) {
        ...
        $shiprocket_status_codes =  $t->config->item('shiprocket_status_codes');
        foreach ($shiprocket_status_codes as $config_status) {
            if ($active_status_code == $config_status['code']) {
                $active_status = $config_status['description'];
            }
            foreach ($track_activities as $track_list) {
                if ($track_list['sr-status'] == $config_status['code']) {
                    $data = [
                        $config_status['description'],
                        $track_list['date'],
                    ];
...
function suggestionKeyword($input_keyword)
{
    ...
    // $subquery1 = "(SELECT TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(p.tags,',', numbers.n),',', -1)) AS suggestion_keyword, p.date_added
    ...
    //            INNER JOIN products p ON CHAR_LENGTH(p.tags) - CHAR_LENGTH(REPLACE(p.tags,',', '')) >= numbers.n - 1
    //            WHERE TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(p.tags,',',numbers.n),',', -1)) LIKE CONCAT('%{$input_keyword}%')
    //           )";

    // $subquery2 = "(SELECT c.name AS suggestion_keyword, p.date_added
    //            FROM products p
    //            JOIN categories c ON c.id = p.category_id
    //            WHERE c.name LIKE CONCAT('%{$input_keyword}%')
    //           )";

    // $subquery3 = "(SELECT p.brand AS suggestion_keyword, p.date_added
    //            FROM products p
    //            WHERE p.brand LIKE CONCAT('%{$input_keyword}%')
    //           )";

    // // Create a union of all three subqueries
    // $union_query = "($subquery1 UNION $subquery2 UNION $subquery3) AS suggestions";

    // $t->db->select('suggestion_keyword')
    //     ->from("($union_query)") // false flag to avoid escaping
    //     ->group_by('suggestion_keyword') // Use GROUP BY to achieve distinct results
    //     ->order_by('date_added', 'DESC')
    //     ->order_by('suggestion_keyword', 'ASC')->limit(8);

    $query = $t->db->query("SELECT `suggestion_keyword` FROM (((SELECT TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(p.tags, ',', numbers.n), ',', -1)) AS suggestion_keyword, 
        p.date_added FROM 
        ...
        INNER JOIN products p ON CHAR_LENGTH(p.tags) - CHAR_LENGTH(REPLACE(p.tags, ',', '')) >= numbers.n - 1 
        WHERE TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(p.tags, ',', numbers.n), ',', -1)) LIKE CONCAT('%$input_keyword%') )
        UNION (SELECT c.name AS suggestion_keyword, p.date_added FROM products p JOIN categories c ON c.id = p.category_id WHERE c.name LIKE CONCAT('%$input_keyword%') )
        UNION (SELECT p.brand AS suggestion_keyword, p.date_added FROM products p WHERE p.brand LIKE CONCAT('%$input_keyword%') )) AS suggestions)
        GROUP BY `suggestion_keyword` ORDER BY `date_added` DESC, `suggestion_keyword` ASC LIMIT 8;");
