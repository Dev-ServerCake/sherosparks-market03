...
<style>
.custom-ok-button {
  background-color: #8b285d !important; /* Your desired background color */
                ...
                <div class="container d-flex flax-wrap is_rtl">
                    ...
                    <div class="d-flex flex-fill">
                        <ul class="align-items-center flex-row ms-auto navbar-nav">
                            <!-- Raghu - Hided the language dropdown -->
                            ...
                            ?>
                                <li class="nav-item dropdown active">
                                    <a class="text-decoration-none" data-toggle="dropdown" href="#"><i class="uil uil-user" style="color: #8b285d;"></i>
                                        <span class="fs-16" style="color: #8b285d;">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-lg">
                                        <a href="<?= base_url('my-account/wallet') ?>" class="dropdown-item text-decoration-none" aria-label="wallet"><i class="uil uil-wallet mr-2 link-color" style="color:#8b285d;"></i> <?= $settings['currency']  ?><?= ' ' . isset($user->balance) && !empty($user->balance) ? number_format($user->balance, 2) : 0.0 ?></a>
                                        <a href="<?= base_url('my-account/profile') ?>" class="dropdown-item text-decoration-none" aria-label="my profile"><i class="uil uil-user link-color mr-2" style="color:#8b285d;"></i> <?= !empty($this->lang->line('profile')) ? $this->lang->line('profile') : 'Profile' ?> </a>
                                        <a href="<?= base_url('my-account/orders') ?>" class="dropdown-item text-decoration-none" aria-label="orders"><i class="link-color mr-2 uil uil-history" style="color:#8b285d;"></i> <?= !empty($this->lang->line('orders')) ? $this->lang->line('orders') : 'Orders' ?> </a>
                                        <a href="" class="dropdown-item text-decoration-none" id="user_logout" aria-label="logout"><i class="link-color mr-2 uil uil-signout" style="color:#8b285d;"></i><?= !empty($this->lang->line('logout')) ? $this->lang->line('logout') : 'Logout' ?></a>
                                    </div>
                                </li>

                            <?php } else { ?>
                                <li class="nav-item active">
                                    <a href="#" aria-label="login" class="mx-1 mb-2 mb-md-0 text-decoration-none hover" style="color: #8b285d;" data-bs-toggle="modal" data-bs-target="#modal-signin"><?= !empty($this->lang->line('login')) ? $this->lang->line('login') : 'Login' ?>
                                    </a>
                                </li>
                                /
                                <li class="nav-item active">
                                    <a href="#" aria-label="register" class="mx-1 mb-2 mb-md-0 text-decoration-none hover" style="color: #8b285d;" data-bs-toggle="modal" data-bs-target="#modal-signup"><?= !empty($this->lang->line('register')) ? $this->lang->line('register') : 'Register' ?>
                                    </a>
                                </li>
                            <?php } ?>
                            <!-- </div> -->
...
<!-- Raghu -->
            <nav class="navbar navbar-expand-lg center-nav transparent navbar-light" style="padding-bottom: 20px;">
                <div class="container flex-lg-row flex-nowrap align-items-center">
                    <div class="navbar-brand w-100 logo-img">
                        <a class="ml-2" href="<?= base_url() ?>">
                            ...
                            <?php } else { ?>
                                <!-- Raghu -->
                                <a href="<?= base_url() ?>"><img src="<?= base_url($web_logo) ?>" data-src="<?= base_url($web_logo) ?>" class="brand-logo-link" alt="site-logo image" style="padding-top: 10px; scale: 2;"></a>
                            <?php } ?>
                        </a>
                    </div>
                            ...
                            <?php if ($page == 'checkout') { ?>
                                <li class="nav-item d-flex flex-row align-items-center">
                                    <a href="<?= base_url('cart') ?>" class="nav-link d-flex" aria-label="cart-image">
                                        ...
                                        <span class="badge bg-primary badge-cart" id='cart-count'><?= (count($this->cart_model->get_user_cart($this->session->userdata('user_id'))) != 0 ? count($this->cart_model->get_user_cart($this->session->userdata('user_id'))) : ''); ?></span>
                                    </a>
                                </li>
            ...
            <div class="offcanvas offcanvas-end bg-light" id="offcanvas-cart" data-bs-scroll="true" aria-modal="true" role="dialog">
                ...
                <div class="offcanvas-footer flex-column text-center container">
                    <a class="btn rounded-pill ms-6 mr-4 mb-5" style="background-color:#fbd0d4;" href="<?= base_url('products') ?>">
                        <?= !empty($this->lang->line('return_to_shop')) ? $this->lang->line('return_to_shop') : 'Return to Shop' ?>
                    </a>

                    <?php if ((isset($user->id)) == 1) { ?>
                        <a href="<?= base_url('cart') ?>" class="btn btn-icon btn-icon-start rounded-pill mb-4 view_cart_button ms-6 mr-4" style="background-color: #8b285d; color:white;">
                            <i class="fs-18 uil uil-shopping-bag"></i>
                            <?= !empty($this->lang->line('view_cart')) ? $this->lang->line('view_cart') : 'View Cart' ?>
                        </a>
                    <?php } else { ?>
                        <a href="#" class="btn rounded-pill btn-icon btn-icon-start mb-4 view_cart_button ms-6 mr-4" style="background-color:#8b285d;color:white;" data-bs-toggle="modal" data-bs-target="#modal-signin">
                            <i class="fs-18 uil uil-shopping-bag"></i>
                            <?= !empty($this->lang->line('view_cart')) ? $this->lang->line('view_cart') : 'View Cart' ?>
                        </a>
                    <?php } ?>
