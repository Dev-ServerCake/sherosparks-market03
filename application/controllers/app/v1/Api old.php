    ...
    */
    ...
    public function get_products()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            // print_r($products);
            $products_total = fetch_product($user_id, null, $product_id, $category_id, '', '', $sort, $order, true, $zipcode, $seller_id, 0);
...
<<<<<<< HEAD
        log_message('debug', $this->response);
=======
        // Raghu added log message
        log_message('debug', 'Response Data: ' . print_r($this->response, true));
>>>>>>> fe4f662 (local server commit after public_html are added after removed)
        print_r(json_encode($this->response));
    }
  ...
  */
    ...
    public function get_promo_codes()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            // $cart_total_data = get_cart_total($user_id);
            // $cod_allow_array = [];

            // foreach ($cart_total_data as $item) {
            //     if (isset($item['cod_allowed'])) {
            //         $cod_allow_array[] = $item['cod_allowed'];
            //     }
            // }
            // $is_cod_on = (in_array(1, $cod_allow_array)) ? 1 : 0;
